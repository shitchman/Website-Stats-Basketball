...
function EditBuild({ setBuilds, onClose }) {
   ...
   const handleDeleteBuild = async () => {
      ...
      try {
         const response = await apiFetch(`/builds/deleteUserBuild?build_id=${selectedBuildId}`, {
            method: 'DELETE',
         });
         ...
         if (!buildsListResponse.ok) {
            setAlertMessage(`Build deleted successfully, but failed to refresh builds list (${buildsListResponse.status}).`);
            setShowAlert(true);
            return;
         }

         const updatedBuilds = await buildsListResponse.json();
         ...
         setAlertMessage('Unable to delete the build. Please try again later.');
         setShowAlert(true);
      } finally {
